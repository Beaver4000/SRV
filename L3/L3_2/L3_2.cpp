...
static int prior_read ( resmgr_context_t *ctp, io_read_t *msg, RESMGR_OCB_T *ocb ) {
	...
	if (msq->i.xtype & _IO_XTYPE_MASK != _IO_XTYPE_NONE) return ENOSYS;
	if (odd){
		struct sched_param param;
		sched_getparam(0, &param);
		static char rbuf [4];
		sprintf(rbuf, "Kuzmin Alexander I992 %d\n", param.sched_curpriority);
		MsgReply(ctp->rcvid, strlen (rbuf) +1, rbuf, strlen(rbuf) +1);
	}
	else MsgReply(ctp->revid, EOK, NULL, 0);
	odd = !odd;
...
id = resmgr_attach (dpp, &resmgr_attr, "/dev/prior", FTYPE_ANY, 0 , &connect_funcs, &io_funcs, &attr);
